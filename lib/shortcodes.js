...
const processLexs = (lexs, headerLvl) => {
  ...
  for (let index = 0; index < lexs.length; index++) {
    ...
    output += ` <pre><code class="language.json language-json">` + JSON.stringify(aLex, null, 2) + '</code></pre>\n';
  }
  return output + '\n'//```';
};
...
const getAllExampleOutput = (allLexes, headerLvlNumber) => {
  ...
  console.log(Object.keys(examples).length, ' examples generated for lex count ', allLexes.length);
